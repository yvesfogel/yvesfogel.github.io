...
function setup() {
    ...
    model = new rw.HostedModel({
        url: "https://landscapes-ba8a260b.hosted-models.runwayml.cloud/v1/",
        token: "IEDApiCIGaT5+o8geWAxEg=="
    });
...
function talkToRunway(reason, vector, angle) {
    ...
    model.query(data).then(outputs => {
        ...
        loader = new THREE.TextureLoader().load( image, texture => {
...
function init3D() {
    ...
    panotexture = new THREE.TextureLoader().load("itp.jpg");
    backMaterial = new THREE.MeshBasicMaterial({ map: panotexture });

    back = new THREE.Mesh(bgGeometery, backMaterial);
    scene.add(back);
